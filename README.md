...
Every dependency is permissively licensed — nothing GPL, LGPL-only, AGPL or
source-available anywhere in the tree. [`THIRD-PARTY.md`](THIRD-PARTY.md)
carries the notices, generated by `cargo about generate about.hbs -o
THIRD-PARTY.md`; `about.toml` lists the accepted licences and fails the run on
anything else rather than writing it in quietly.

The workbooks under `tests/fixtures/vendor/` come from the calamine test suite
and are MIT — see
[`LICENSE-calamine.md`](tests/fixtures/vendor/LICENSE-calamine.md) beside them.
The embedding model downloaded at runtime,
[`bge-small-en-v1.5`](https://huggingface.co/BAAI/bge-small-en-v1.5), is MIT,
as is the ONNX Runtime that executes it.
