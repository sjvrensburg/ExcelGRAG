...
impl<RS: Read + Seek> Xls<RS> {
    fn parse_workbook(&mut self) -> Result<(), XlsError> {
        ...
        let name_scopes = defined_names.iter().map(|(_, scope, _)| *scope).collect();
        ...
        self.metadata.name_scopes = name_scopes;
...
fn parse_lbl(
    ...
    encoding: &XlsEncoding,
    biff: Biff,
) -> Result<(String, Option<usize>, (Option<usize>, String)), XlsError> {
    ...
    // Lbl.itab is 0 for workbook scope and otherwise a one-based tab index.
    let itab = read_u16(&r.data[8..10]) as usize;
    Ok((
        name,
        itab.checked_sub(1).filter(|&sheet| sheet < 65_536),
        formula,
    ))
}
    ...
    while !rgce.is_empty() {
        ...
        match ptg {
            ...
            0x3b | 0x5b | 0x7b => {
                ...
                let first_col = read_u16(&rgce[6..8]);
                let last_col = read_u16(&rgce[8..10]);
                if first_col & 0x4000 == 0 {
                    formula.push('$');
                }
                push_column((first_col & 0x3fff) as u32, &mut formula);
                if first_col & 0x8000 == 0 {
                    formula.push('$');
                }
                write!(&mut formula, "{}", first_row as u32 + 1).unwrap();
                if first_row != last_row || first_col != last_col {
                    formula.push(':');
                    if last_col & 0x4000 == 0 {
                        formula.push('$');
                    }
                    push_column((last_col & 0x3fff) as u32, &mut formula);
                    if last_col & 0x8000 == 0 {
                        formula.push('$');
                    }
                    write!(&mut formula, "{}", last_row as u32 + 1).unwrap();
                }
                rgce = &rgce[10..];
            }
            0x3c | 0x5c | 0x7c => {
