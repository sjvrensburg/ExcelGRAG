...
use crate::{
    CellErrorType, Data, DataType, HeaderRow, Metadata, Range, Reader, Sheet, SheetType,
    SheetVisible,
};
    ...
    RS: Read + Seek,
{
    ...
    fn new(reader: RS) -> Result<Self, OdsError> {
        ...
        let metadata = Metadata {
            ...
            name_scopes: vec![None; defined_names.len()],
            names: defined_names,
        };
    ...
    RS: Read + Seek,
{
    ...
    for attr in e.iter_raw_attrs() {
        ...
        match key {
            ...
            // LibreOffice uses this extension because ODF has no standard
            // error value type. The error text lives in the element body.
            b"calcext:value-type" if value == b"error" => is_error = true,
            b"table:formula" => {
    ...
    if is_error || (!is_value_set && is_string) {
