...
pub struct Metadata {
    ...
    /// Sheet scope for each defined name, by zero-based sheet index. `None`
    /// means workbook scope. Parallel with `names` for API compatibility.
    name_scopes: Vec<Option<usize>>,
}
    ...
    RS: Read + Seek,
{
    ...
    /// Sheet scope for each entry returned by [`Reader::defined_names`].
    fn defined_name_scopes(&self) -> &[Option<usize>] {
        &self.metadata().name_scopes
    }
