...
#[test]
fn importance_off_reproduces_the_ordering_from_before_the_field_existed() {
    // Main reads Obscure and Popular from exactly one cell each, so the two
    // steps tie on hop distance and edge weight. `importance_weight: None`
    // (the default) must break that tie exactly as it did before importance
    // existed: by node index, never by the score.
    let wb = tie_workbook();
    let (_root, corpus) = corpus_of("tie-off", &[wb]);
    let seed = hit_for(&corpus, "hash-tie", "Main", NodeKind::Sheet);
    ...
    let sheets: Vec<&str> = found.workbooks[0]
        ...
        .filter_map(|n| n.sheet.as_deref())
        .collect();
    // Obscure is declared before Popular, so it carries the lower node
    // index and wins the tie when importance plays no part.
    assert!(sheets.contains(&"Obscure"), "got {sheets:?}");
    assert!(!sheets.contains(&"Popular"), "got {sheets:?}");
}

#[test]
fn importance_breaks_the_same_tie_the_other_way_once_enabled() {
    // Same workbook, same tie — but Popular is also read by three other,
    // independent tables, so its cached importance score is higher than
    // Obscure's. With `importance_weight` set, that score decides the tie
    // instead of node index, and the more-referenced sheet wins.
    let wb = tie_workbook();
    let (_root, corpus) = corpus_of("tie-on", &[wb]);
    let seed = hit_for(&corpus, "hash-tie", "Main", NodeKind::Sheet);
...
/// Main reads Obscure and Popular from exactly one cell each — a tie on hop
/// distance and edge weight. Three other sheets read only Popular, so its
/// importance score is higher than Obscure's, which has no other referrer.
fn tie_workbook() -> Workbook {
    Workbook {
        path: "tie.xlsx".into(),
        format: Some(WorkbookFormat::Xlsx),
        content_hash: "hash-tie".into(),
        sheets: vec![
            grid(0, "Main", &["Row Value", "a =Obscure!B2", "b =Popular!B2"]),
            grid(1, "Obscure", &["K V", "x 1"]),
            grid(2, "Popular", &["K V", "x 1"]),
            grid(3, "Ref1", &["K V", "x =Popular!B2"]),
            grid(4, "Ref2", &["K V", "x =Popular!B2"]),
            grid(5, "Ref3", &["K V", "x =Popular!B2"]),
        ],
        defined_names: Vec::new(),
        external_links: Vec::new(),
    }
