...
Why ExcelGRAG is built the way it is: the measurements behind the constants,
the approaches that were tried and discarded, and the trades that are not
obvious from the code. `README.md` says what the project does and how to run
it; this says why it does it that way.

It is written in the order a question travels — reader, structure, graph,
index, retrieval, cells — and it is a working record rather than a
specification. Where a number settled a decision, the number is named. Where
something was measured and turned out not to matter, that is here too, because
a constant nobody can explain is a constant nobody can change.

**The figures are not in this file.** They were measured against a real
workbook that is confidential, so each example prints its own on the terminal,
which is where they belong. Sheet names appear as pseudonyms, consistently, so
a name that recurs is the same sheet. The generated demo workbook under
`tests/fixtures/demo` is synthetic and committed, and its figures *are* quoted
freely — it is the one you can run all of this against yourself:
...
See also [`upstream-issues.md`](upstream-issues.md) for the ten calamine
defects found while building this, and `CLAUDE.md` at the repository root for
the invariants and working rules that fall out of everything below.
...
- [The graph](#the-graph)
- [Reading a region as a table](#reading-a-region-as-a-table)
- [What the columns hold](#what-the-columns-hold)
- [Asking a table a question](#asking-a-table-a-question)
- [The schema a workbook writes down without meaning to](#the-schema-a-workbook-writes-down-without-meaning-to)
- [The corpus](#the-corpus)
- [The lexical index](#the-lexical-index)
- [The vector index](#the-vector-index)
- [Retrieval](#retrieval)
- [Cell-level provenance](#cell-level-provenance)
...
The tool list and how to wire it up are in the README; this is why it is a
server rather than a command, and how the protocol is written.
...
they fail differently, which is the whole reason there are several.
